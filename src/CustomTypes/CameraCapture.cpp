...
void CameraCapture::Init(CameraRecordingSettings const& settings) {
    capture = std::make_unique<MediaCodecEncoder>(
        readOnlyTexture->get_width(), readOnlyTexture->get_height(), settings.fps, settings.bitrate, settings.filePath
    );
...
void CameraCapture::Update() {
    if (!(capture->isInitialized() && readOnlyTexture && readOnlyTexture->m_CachedPtr != nullptr))
        return;
...
bool CameraCapture::HandleFrame(AsyncGPUReadbackPlugin::AsyncGPUReadbackPluginRequest* req) {
    ...
    if (capture->isInitialized() && readOnlyTexture->m_CachedPtr != nullptr) {
        req->Update();
